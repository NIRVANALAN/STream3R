# https://raw.githubusercontent.com/naver/mast3r/refs/heads/main/mast3r/losses.py
...
class Regr3D_ScaleInv(Regr3D):
    ...
    def get_all_pts3d(self, gt1, gt2, pred1, pred2):
        ...
        # measure scene scale
        _, gt_scale = get_joint_pointcloud_center_scale(
            gt_pts1, gt_pts2, mask1, mask2)
...
class InfoNCE(MatchingCriterion):
    ...
    def loss(self, desc1, desc2, valid_matches=None, euc=False):
        ...
        assert valid_matches.shape == torch.Size(
            [B, N]) and valid_matches.sum() > 0
...
class MatchingLoss(Criterion, MultiLoss):
    ...
    def add_negatives(self, outdesc2, desc2, batchid, x2, y2):
        if self.negatives_padding:
            ...
            outdesc2 = torch.cat([outdesc2, desc2[sel].view([B, -1, D])],
                                 dim=1)
        return outdesc2
...
class ConfMatchingLoss(ConfLoss):
    ...
    def aggregate_confs(
        ...
        if self.confmode == 'prod':
            ...
            confs = .5 * (
                confs1 +
                confs2) if confs1 is not None and confs2 is not None else 1.
        else:
