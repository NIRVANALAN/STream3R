...
class ConfLoss_MV(MultiLoss):
    ...
    def compute_loss(self, gts, preds, **kw):
        ...
        if "pose_loss" in details:
            ...
            #               )  # , details
            final_loss = (final_loss + details["pose_loss"])  # ! vggt does not weight this loss term
...
class Regr3D_MV(Regr3D):
    ...
    def get_norm_factor_point_cloud(self,
        ...
        if norm_self_only: # always false.
            ...
            pts = [
                torch.cat([x, y], dim=2) for x, y in zip(pts_self, pts_cross) # B H W*2 3
            ]
    ...
    def get_all_pts3d(
        ...
        gt_pts_cross = [geotrf(in_camera1, gt["pts3d"]) for gt in gts]  # world
        ...
        # if preds[0]["conf_self"].shape != preds[0]['conf'].shape: # ?
        #     st()
        ...
        norm_factor_pr = norm_factor_gt.clone()  # ! if using self.gt_scale
        norm_factor_crossonly_pr = norm_factor_gt.clone()  # ! if using self.gt_scale
        if self.norm_mode and not_metric_mask.sum() > 0 and not self.gt_scale:
            ...
            norm_factor_pr[not_metric_mask] = norm_factor_pr_not_metric
            norm_factor_crossonly_pr[not_metric_mask] = norm_factor_pr_not_metric2
        ...
        # print('debugging norm_factor_gt !!! ')
        # gt_pts_cross = [pts / norm_factor_gt2 for pts in gt_pts_cross]
        # pr_pts_cross = [pts / norm_factor_pr for pts in pr_pts_cross]

        # st()
        # output_path = 'examples/co3d-debug-coord2'
        # output_path = 'examples/co3d-debug-coord3'
        # output_path = 'examples/co3d-debug-coord3'
        # for v_idx in range(len(pr_pts_cross)):
        #     save_pts3d(pr_pts_cross[v_idx][0:1], 
        #                output_path,
        #                 output_suffix=f'{v_idx}-normalized-cross-pred.obj')
        #     save_pts3d(gt_pts_cross[v_idx][0:1], 
        #                output_path,
        #                 output_suffix=f'{v_idx}-normalized-cross-gt.obj')
        # st()

        # [(Bx3, BX4), (BX3, BX4), ...], 3 for translation, 4 for quaternion
        if self.has_cam_loss:
            # st()
            # gt_poses = [ # ! if prediction E only, cut3r.
            #     camera_to_pose_encoding(in_camera1 @ gt["camera_pose"]).clone()
            #     for gt in gts
            # ]

            # ! if prediction camera [E|K]
            gt_poses = extri_intri_to_pose_encoding(
                torch.stack([in_camera1 @ gt["camera_pose"] for gt in gts],
                            dim=1),
            ...
            # extri_intri_to_pose_encoding(in_camera1 @ gt["camera_pose"], gt["camera_intrinsics"]).clone()

            pose_norm_factor_gt = norm_factor_gt.clone().squeeze(2, 3)
            pose_norm_factor_pr = norm_factor_pr.clone().squeeze(2, 3)

            # st()
            # ! expand the pose prediction for supervising all intermedaite steps
            if isinstance(preds[0]["camera_pose"],
                          list):  # iterative prediction
                ...
                for iteration_step in range(num_of_pose_iteration):
                    for view_idx in range(len(gts)):
                        pr_poses.append(preds[view_idx]['camera_pose']
                                        [iteration_step].clone())
            ...
            # st()
            pose_masks = (pose_norm_factor_gt.squeeze(1)
                          > eps) & (pose_norm_factor_pr.squeeze(1) > eps)

            # ! no need to use camera_only?
            if any(camera_only):
    ...
    def compute_loss(self, gts, preds, **kw):
        ...
        if "Quantile" in self.criterion.__class__.__name__:
            ...
            ls_self, masks = self.criterion(pred_pts_self, gt_pts_self, masks,
                                            gts[0]["quantile"])
        else:
            ls_self = [
                self.criterion(pred_pt[mask],
                               gt_pt[mask]) for pred_pt, gt_pt, mask in zip(
                                   pred_pts_self, gt_pts_self, masks)
            ]
