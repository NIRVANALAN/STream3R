...
# from fast3r.cotracker.models.core.cotracker.losses import huber_loss
from dust3r.utils.geometry import inv
from model.vggt.utils.pose_enc import extri_intri_to_pose_encoding
...
def camera_loss(pred_pose_enc_list,
                gt_extrinsic,
                gt_intrinsic,
                image_size_hw,
                gt_pts3d_scale,
    ...
    gt_extrinsic_aligned = anchor_camera_inv @ gt_extrinsic
    gt_pose_encoding = extri_intri_to_pose_encoding(
        gt_extrinsic_aligned,
        gt_intrinsic,
        image_size_hw,
        pose_encoding_type=pose_encoding_type,
        gt_pts3d_scale=gt_pts3d_scale)
    ...
    for i in range(num_predictions):
        ...
        if frame_num > 0:
            loss_T_i, loss_R_i, loss_fl_i = camera_loss_single(
                cur_pred_pose_enc[:, :frame_num].clone(),
                gt_pose_encoding[:, :frame_num].clone(),
                loss_type=loss_type)
        else:
            loss_T_i, loss_R_i, loss_fl_i = camera_loss_single(
                cur_pred_pose_enc.clone(),
...
def camera_loss_single(cur_pred_pose_enc, gt_pose_encoding, loss_type="l1"):
    if loss_type == "l1":
        ...
        loss_T = huber_loss(cur_pred_pose_enc[..., :3],
                            gt_pose_encoding[..., :3],
                            delta=0.01)
        loss_R = huber_loss(cur_pred_pose_enc[..., 3:7],
                            gt_pose_encoding[..., 3:7],
                            delta=0.01)
        loss_fl = huber_loss(cur_pred_pose_enc[..., 7:],
                             gt_pose_encoding[..., 7:],
                             delta=0.1)
    else:
...
def depth_loss(depth,
    ...
    if loss_type == "conf":
        conf_loss_dict = conf_loss(depth,
                                   depth_conf,
                                   gt_depth,
                                   valid_mask,
                                   batch,
...
def point_loss(pts3d,
    ...
    conf_loss_dict = conf_loss_fn(
        ...
        temporal_matching_loss=temporal_matching_loss)

    return conf_loss_dict


def filter_by_quantile(loss_tensor,
                       valid_range,
                       min_elements=1000,
                       hard_max=100):
    """
...
def conf_loss(pts3d,
    ...
    if normalize_gt:
        if gt_pts3d_scale is None:
            # st() # no need to normalize metric-scale datasets
            if metric_flag is not None:
                # Only normalize the gt_pts3d that is not metric
                # st()
                non_metric_mask = ~metric_flag
                gt_pts3d_non_metric = gt_pts3d[non_metric_mask]
                valid_mask_non_metric = valid_mask[non_metric_mask]
                
                # Normalize non-metric points
                gt_pts3d_non_metric, gt_pts3d_scale_non_metric = normalize_pointcloud(
                    gt_pts3d_non_metric, valid_mask_non_metric)
                
                # Put normalized points back
                gt_pts3d[non_metric_mask] = gt_pts3d_non_metric
                gt_pts3d_scale = torch.ones_like(metric_flag, dtype=gt_pts3d.dtype)
                gt_pts3d_scale[~metric_flag] = gt_pts3d_scale_non_metric
            else:
                gt_pts3d, gt_pts3d_scale = normalize_pointcloud(
                    gt_pts3d, valid_mask)
        else:
    ...
    if disable_conf:
        ...
        conf_loss_first_frame = gamma * loss_reg_first_frame * first_frame_conf[
            first_frame_mask] - alpha * torch.log(
                first_frame_conf[first_frame_mask])
        conf_loss_other_frames = gamma * loss_reg_other_frames * other_frames_conf[
            other_frames_mask] - alpha * torch.log(
                other_frames_conf[other_frames_mask])

    if valid_range > 0:
        conf_loss_first_frame = filter_by_quantile(conf_loss_first_frame,
                                                   valid_range)
        conf_loss_other_frames = filter_by_quantile(conf_loss_other_frames,
                                                    valid_range)
    ...
    if all_mean and conf_loss_first_frame.numel(
    ) > 0 and conf_loss_other_frames.numel() > 0:
    ...
    loss_dict = {
        ...
        conf_loss,
        f"loss_reg1{postfix}":
        loss_reg_first_frame.detach().mean()
        if loss_reg_first_frame.numel() > 0 else 0,
        f"loss_reg2{postfix}":
        loss_reg_other_frames.detach().mean()
        if loss_reg_other_frames.numel() > 0 else 0,
        f"loss_conf1{postfix}":
        conf_loss_first_frame,
        f"loss_conf2{postfix}":
...
def reg_loss(pts3d,
    ...
    loss_reg_first_frame = torch.norm(first_frame_gt_pts3d[first_frame_mask] -
                                      first_frame_pts3d[first_frame_mask],
                                      dim=-1)
    ...
    loss_reg_first_frame = check_and_fix_inf_nan(loss_reg_first_frame,
                                                 "loss_reg_first_frame")
    loss_reg_other_frames = check_and_fix_inf_nan(loss_reg_other_frames,
    ...
    if temporal_matching_loss:
        ...
        loss_temporal_matching = F.l1_loss(pred_diff[valid_mask],
                                           gt_diff[valid_mask],
                                           reduction='none')
        valid_count = valid_mask.sum()
        loss_temporal_matching = (loss_temporal_matching.sum() /
                                  valid_count) if valid_count > 0 else 0
    else:
...
def normal_loss(prediction, target, mask, cos_eps=1e-8, conf=None):
    ...
    """
    pred_normals, pred_valids = point_map_to_normal(prediction,
                                                    mask,
                                                    eps=cos_eps)
...
def point_map_to_normal(point_map, mask, eps=1e-6):
    ...
    with torch.cuda.amp.autocast(enabled=False):
        ...
        v1 = padded_mask[:, :-2,
                         1:-1] & padded_mask[:, 1:-1,
                                             1:-1] & padded_mask[:, 1:-1, :-2]
        v2 = padded_mask[:, 1:-1, :-2] & padded_mask[:, 1:-1,
                                                     1:-1] & padded_mask[:, 2:,
                                                                         1:-1]
        v3 = padded_mask[:, 2:,
                         1:-1] & padded_mask[:, 1:-1,
                                             1:-1] & padded_mask[:, 1:-1, 2:]
        v4 = padded_mask[:, 1:-1, 2:] & padded_mask[:, 1:-1,
                                                    1:-1] & padded_mask[:, :-2,
                                                                        1:-1]
...
def gradient_loss_multi_scale(prediction,
                              target,
                              mask,
                              scales=4,
