...
class RNN_Window(PoseEvaluatorWindow):
    ...
    def inference(self, batch) -> list:
        ...
        gaussians = self.encoder(  # means: 1, HxWxV, 3
            ...
            self.global_step,
            visualization_dump=
            visualization_dump,  # returns in: 1, 9, 256, 256, 1, 3
        )
        return gaussians, visualization_dump

    def get_reconstructed_scene(self,
        ...
        raise NotImplementedError(
            "Windowed global aligner output conversion is not implemented for RNN_Window."
        )
    ...
    def _merge_two_gaussians(self,
                             gaussian_prev,
                             gaussian_cur,
                             merge_shared_gs=False):
        ...
        if merge_shared_gs:  # remove the "aligned src" pifu gaussians
            ...
            gaussian_cur = Gaussians(
                torch.cat([gaussian_cur.means[0:1], gaussian_cur.means[2:]],
                          0),  # 8 N C
                torch.cat([
                    gaussian_cur.covariances[0:1], gaussian_cur.covariances[2:]
                ], 0),
                torch.cat(
                    [gaussian_cur.harmonics[0:1], gaussian_cur.harmonics[2:]],
                    0),
                torch.cat(
                    [gaussian_cur.opacities[0:1], gaussian_cur.opacities[2:]],
                    0),
            )

        merged_gaussian = Gaussians(
            torch.cat([gaussian_prev.means, gaussian_cur.means], 0),
            torch.cat([gaussian_prev.covariances, gaussian_cur.covariances],
                      0),
    ...
    def _to_pixelaligned_gs(self, gaussians, V):
        ...
        pixel_aligned_gaussians = Gaussians(
            rearrange(gaussians.means.float().cpu(),
            ...
            rearrange(gaussians.harmonics.float().cpu(),
                      "b (v r srf spp) c d_sh -> (b v) (r srf spp) c d_sh",
                      v=V,
                      srf=srf,
                      spp=spp),
            rearrange(gaussians.opacities.float().cpu(),
    ...
    def test_step(self, batch, batch_idx):
        ...
        batch_of_windows = [self.data_shim(item) for item in batch
                            ]  #  batch is a list of BatchedExample here

        all_reconstruction = []

        view1, view2, pred1, pred2 = [
            defaultdict(list) for _ in range(4)
        ]  # match the format of cloud_opt in dust3r

        # * remap index (dust3r global alignment only accepts 0-N unbroken sequences as the index)
        all_index = []
        for batch in batch_of_windows:
            all_index.extend(
                batch['context']['index'][0].cpu().numpy().tolist())

        all_index_set = sorted(set(all_index))
        index_mapping = {
            frame_idx: order_idx
            for order_idx, frame_idx in enumerate(all_index_set)
        }
        ...
        for window_id, batch in enumerate(batch_of_windows):
            ...
            # ! in hybrid training setting
            # print("!!!!!normalizing context image to [-1,1] for hybrid ckpt.")
            # st()
            # batch["context"]['image'] = batch["context"][
            #     'image'] * 2 - 1  # [0,1] -> [-1,1], to align with dust3r datasets
            ...
            input_images_view2 = rearrange(
                batch["context"]["image"][:, 1:].clone(),
                "b v ... -> (b v) 1 ...",
            )
            ...
            visualization_dump['ctx_imgs_collate'] = rearrange(
                batch["context"]["image"][:].clone(),
                "b v rgb h w -> b (v h w) rgb").detach().cpu().numpy()

            visualization_dump['ctx_imgs'] = rearrange(
                batch["context"]["image"][:].clone(),
                "b v rgb h w -> (b v) (h w) rgb"  # b=1 here, so it's fine
            ).detach().cpu().numpy()

            visualization_dump['index'] = batch['context']['index'][0].cpu(
            ).numpy().tolist()
            visualization_dump['ga_index'] = [
                index_mapping[idx] for idx in visualization_dump['index']
            ]
            ...
            window_rec_result = [
                gaussians, pixel_aligned_gaussians, visualization_dump
            ]
            ...
            if render_flag:  # render the video
                ...
                if False:  # observe fixRs2 performance
                    ...
                    if global_window_batch is not None:  # ! if to render global trajectory
                        ...
                        video_name = f'window-{window_id}-rnn_fixCovRs2'

                        # if procrustes_alignment and window_id > 0:
                        #     if not joint_render_flag:
                        #         gaussians_to_render = aligned_gaussians
                        #     else:
                        #         video_name = f'window-{window_id}-rnn-joint'
                    ...
                    # st()
                    images = [
                        vcat(gt_rgb, rgb, depth) for rgb, gt_rgb, depth in zip(
                            output.color[0].cpu(), gt[0].cpu(),
                            vis_depth_map(output.depth[0].cpu()))
                    ]

                    video = torch.stack(images)
                    video = (video.clip(min=0, max=1) * 255).type(
                        torch.uint8).cpu().permute(0, 2, 3,
                                                   1).numpy()  # F H W 3
            ...
            # * set view info
            view1['true_shape'].extend([
                torch.tensor(batch['context']['image'].shape[-2:])
                for _ in range(v)
            ])
            view2['true_shape'].extend([
                torch.tensor(batch['context']['image'].shape[-2:])
                for _ in range(v)
            ])
            ...
            view1['img'].append(
                batch['context']['image'][0, 0:1].repeat_interleave(
                    v, dim=0))  # v-1, h, w, 3. for concat later.
            ...
            view1['idx'].extend(
                [batch['context']['index'][0, 0].item() for _ in range(v)])
            view2['idx'].extend(
                batch['context']['index'][0, :].cpu().numpy().tolist())
            ...
            pred1['pts3d'].append(pixel_aligned_gaussians.means[0:1].float().
                                  cpu().repeat_interleave(v, dim=0))
            pred2['pts3d'].append(
                pixel_aligned_gaussians.means[:].float().cpu())
            ...
            pred1['conf'].append(pixel_aligned_gaussians.opacities[0:1].float(
            ).cpu().repeat_interleave(v, dim=0))
            pred2['conf'].append(pixel_aligned_gaussians.opacities[:].float().
                                 cpu())  # directly use opacities as the conf.

        # * collate the output
        def collate_window_prediction(view1, view2, pred1, pred2):

            def collate_view(view):
                view['true_shape'] = torch.stack(
                    view['true_shape'], 0)  # other stuffs are already extended
                view['img'] = torch.cat(view['img'],
                                        0)  # other stuffs are already extended
                # do the re-mapping
                view['idx'] = [index_mapping[idx] for idx in view['idx']]
                return view

            def collate_pred(pred):  # dict contains torch.Tensor
                pred['pts3d'] = rearrange(torch.cat(pred['pts3d'], dim=0),
                                          'b (h w) ... -> b h w ...',
                                          h=h,
                                          w=w)
                pred['conf'] = rearrange(torch.cat(pred['conf'], dim=0),
                                         'b (h w) ... -> b h w ...',
                                         h=h,
                                         w=w)
                return pred
        ...
        view1, view2, pred1, pred2 = collate_window_prediction(
            view1, view2, pred1, pred2)
        ...
        # output_dir = './outputs_cpfs/rnn_co3d/v0_10k-ckpt'

        # output_dir = './outputs_cpfs/re10k_dl3dv_ar_causal_v1.5/v0_20k'
        # output_dir = './outputs_cpfs/re10k_dl3dv_ar_causal_v1.5/v8_10k'
        # output_dir = './outputs_cpfs/re10k_dl3dv_ar_causal_v1.5/v8_30k-collapsed'
        # output_dir = './outputs_cpfs/re10k_dl3dv_ar_causal_v1.6/v8_40k-not_collapsed'
        # output_dir = './outputs_cpfs/re10k_dl3dv_ar_causal_v1.6/v2_70k-not_collapsed'
        # output_dir = './outputs_cpfs/re10k_dl3dv_ar_causal_v1.5/v9_10k-Not_Yet_collapsed'
        # output_dir = './outputs_cpfs/re10k_dl3dv_ar_causal_v1.6/v8_5k-Not_Yet_collapsed'
        # output_dir = './outputs_cpfs/croco_multi_rw_ar_causal_v1/v0_35k'
        # output_dir = './outputs_cpfs/croco_multi_rw_ar_causal_v1/v0_35k'
        # st() # check which layer nan output

        # output_dir = './outputs_cpfs/re10k_ar_causal_v1.7-must3r/v8_30k-not_collapsed'
        # output_dir = './outputs_cpfs/must3r/co3d/v8_15k'
        # output_dir = './outputs_cpfs/must3r/co3d/v8_15k_no-gs-finetune_rolloutTo16'
        # output_dir = './outputs_cpfs/must3r/co3d/v2_75k_gs-finetune'
        # output_dir = './outputs_cpfs/must3r/co3d/v2_20k_no-gs-finetune'
        # output_dir = './outputs_cpfs/must3r/co3d/v9_Feb12nd_FA_150K'

        # output_dir = './outputs_cpfs/must3r/co3d/v8_15K_middle-view'

        # output_dir = './outputs_cpfs/must3r/co3d/v8_15K_middle-view'
        # output_dir = './outputs_cpfs/must3r/co3d/v8_15k_no-gs-finetune_rolloutTo16_fixRender'

        # output_dir = './outputs_cpfs/must3r/re10k/v8_50k_v=8_pts_pretrain'
        # output_dir = './outputs_cpfs/must3r/re10k/v8_20k_v=8_pts_pretrain_only'
        # output_dir = './outputs_cpfs/must3r/re10k/v8_40k_v=8_pts_v2WarmUp-drop20percent'
        # output_dir = './outputs_cpfs/must3r/re10k/v8_20k_v=8_pts_v2WarmUp-drop20percent'
        # output_dir = './outputs_cpfs/must3r/re10k/v8_20k_v=8_pts_v2WarmUp-drop20percent-testV4perf'
        # output_dir = './outputs_cpfs/must3r/re10k/v8_20k_v=4_pts_v2WarmUp-drop20percent-forcev3'
        # output_dir = './outputs_cpfs/must3r/re10k/v8_20k_v=4_pts_v2WarmUp-drop20percent-forcev3-v2_perf'
        # output_dir = './outputs_cpfs/must3r/re10k/v8_20k_v=8_pts_v2WarmUp-drop20percent-forcev3-%7'

        # output_dir = './outputs_cpfs/must3r/re10k/v8_20k_v=8_pts_v2WarmUp-drop20percent-forcev2-v2_perf'
        # output_dir = './outputs_cpfs/must3r/re10k/joint_training/v4_45K'
        # output_dir = './outputs_cpfs/must3r/re10k/joint_training/v4_15K'
        # output_dir = './outputs_cpfs/must3r/re10k/no_gs_finetune/v4_360k'
        # output_dir = './outputs_cpfs/must3r/re10k/joint_training/v4_7.5k'
        # output_dir = './outputs_cpfs/must3r/re10k/joint_training/v2_10k'
        # output_dir = './outputs_cpfs/must3r/re10k/reproduce_noposplat/v2_50k'
        # output_dir = './outputs_cpfs/must3r/re10k/reproduce_noposplat/withK_v2_150k'
        # output_dir = './outputs_cpfs/must3r/re10k/multiview/finetune150K_v=4'
        # output_dir = './outputs_cpfs/must3r/re10k/multiview/jointdust3r_15K_v=4_fulltraj'
        output_dir = './outputs_cpfs/must3r/re10k/multiview/jointdust3r_15K_v=4_fulltraj_basemodel'
        # output_dir = './outputs_cpfs/must3r/re10k/reproduce_noposplat/v2_testv4_50k'
        # output_dir = './outputs_cpfs/must3r/re10k_dl3dv/reproduce_noposplat/v2_45k'

        os.makedirs(output_dir, exist_ok=True)

        def vis_gs_pcd(rec_result,
                       output_path,
                       pcd_name_prefix,
                       save_rgb=False,
                       verb=False,
                       save_different_view=False):
            # ! visualization of the predicted gaussians
            # gaussians, pixel_aligned_gaussians, visualization_dump = rec_result
            gaussians, pixel_aligned_gaussians, visualization_dump = rec_result[
                0], rec_result[1], rec_result[2]

            if save_different_view:
                for v in range(pixel_aligned_gaussians.means.shape[0]):
                    gs_xyz = pixel_aligned_gaussians.means[v]
                    gs_rgb = (visualization_dump['ctx_imgs'][v] * 127.5 +
                              127.5).astype(np.uint8)  # V 1 3 H W
                    frameidx = visualization_dump['index'][v]
                    pcd = trimesh.Trimesh(vertices=gs_xyz,
                                          vertex_colors=gs_rgb)

                    pcd_output_path = f'{pcd_name_prefix}-v_{v}-frameidx_{frameidx}.obj'
                    pcd.export(os.path.join(output_path, pcd_output_path),
                               'obj')
                ...
                if save_rgb:
                    gs_rgb = (
                        visualization_dump['ctx_imgs_collate'][0] * 127.5 +
                        127.5).astype(np.uint8)  # V 1 3 H W
                    pcd = trimesh.Trimesh(vertices=gs_xyz,
                                          vertex_colors=gs_rgb)
                else:
                ...
                pcd.export(os.path.join(output_path, f'{pcd_name_prefix}.obj'),
                           'obj')
        ...
        def save_video(video, video_path, verb=False):
            ...
            rgb_video_out = imageio.get_writer(video_path,
                                               mode='I',
                                               fps=24,
                                               codec='libx264')
            ...
            if verb:
                print(
                    f'video of {video.shape[0]} frames saved to {video_path}')
        ...
        for window_id, window_rec_result in enumerate(all_reconstruction):
            ...
            if render_flag:  # save video to local dir
                ...
                vid_output_dir = os.path.join(
                    output_dir, f'{batch_idx}-{video_name}.mp4')  #
                save_video(video, vid_output_dir, verb=True)

            # save pcd
            pcd_name_prefix = f'{batch_idx}-window-{window_id}'
            # vis_gs_pcd(window_rec_result, output_dir,pcd_name_prefix=pcd_name_prefix, save_rgb=True, verb=True)
            vis_gs_pcd(window_rec_result,
                       output_dir,
                       pcd_name_prefix=pcd_name_prefix,
                       save_rgb=True,
                       verb=True,
                       save_different_view=True)
