...
class FramePack3Rv3_Abla_Mem_cut3r_v2init_withf0(FramePack3Rv3):
    ...
    def _rnn_decoder_initialize_v2(self, f_state, f1, pos_1, f2, pos_2, *args, **kwargs):
        # ! initializing causal inference from paired views

        # final_output = [(f1 + self.learnable_frame0_token,
        #                  f2 + self.learnable_B_token)]

        # already added the register tokens
        final_output = [(f1, f2)]
        b = f1.shape[0]
        final_state_output = [f_state] # already initialized

        # final_output = [(f1+0, f2 + self.learnable_B_token)]

        for idx, (blk_siamese, blk_state) in enumerate(
                zip(self.dec_blocks, self.dec_state_blocks)):  # ! 12 blocks of decoder.
        ...
        final_state_output[-1] = self.dec_norm_for_state(final_state_output[-1])
        return zip(*final_output), final_state_output[-1]


    # '''
    def _rnn_decoder_initialize(self, f_state, f1, m1, pos_1, pos_fstate, *args, **kwargs):
        # ! initializing causal inference from paired views
        ...
        for idx, (blk_siamese, blk_state) in enumerate(
            ...
            if self.gradient_checkpointing and self.training and torch.is_grad_enabled():
                def create_custom_forward(module_state, module_siamese):
                    def custom_forward(inputs, f0, pos_fstate, pos_1):
                        f_state, _ = module_state(*inputs[::+1], pos_fstate, pos_1)
                        f1, _ = module_siamese(inputs[1], torch.cat([inputs[0], f0], dim=1), pos_1, pos_fstate)
                        return f_state, f1
                ...
                f_state, _ = blk_state(*final_output[-1][::+1], pos_fstate, pos_1)
                f1, _ = blk_siamese(final_output[-1][1], torch.cat([final_output[-1][0], m1[idx]], dim=1), pos_1, pos_fstate)
    ...
    def _forward_decoder_step(
        ...
        (m1, m2), f_state = self._rnn_decoder_initialize_v2(f_state,feat[:, 0], pos[:, 0], feat[:, 1], pos[:, 1])
        ress.append(m1)
        ress.append(m2)

        # ! only state attention
        if v >2:
            for i in range(2, v):
                f_state_all, output = self._rnn_decoder_initialize(f_state, feat[:, i], m1, None, None)
