...
class FramePack3Rv3_Abla_FA_with_state(FramePack3Rv3):
    ...
    def _rnn_decoder_initialize(self, f1, pos_1, f2, pos_2, *args, **kwargs):
        ...
        final_memory = [self.memory_token.expand(f2.shape[0], -1, -1)]
        ...
        for idx, blk_siamese in enumerate(
            ...
            # st()
            f1, _ = blk_siamese(
                torch.cat([final_output[-1][:, 1], final_memory[-1]], dim=1),
            ...
            # st()
            f1, f_state = f1[:, :l], f1[:, l:]
            final_memory.append(f_state)
        ...
        return final_output, final_memory[-1]
    ...
    def _forward_decoder_step(
        ...
        ress, _ = self._rnn_decoder_initialize(feat[:, 0], None,
                                                     feat[:, 1:], None)
