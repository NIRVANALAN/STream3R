...
class FramePack3Rv3_Abla_Mem_cut3r_v2init(FramePack3Rv3):
            ...
            if self.gradient_checkpointing and self.training and torch.is_grad_enabled():
                def create_custom_forward(module_state, module_siamese):
                    def custom_forward(inputs, pos_fstate, pos_1):
                        ...
                        f1, _ = module_siamese(*inputs[::-1], pos_1, pos_fstate)
                        return f_state, f1
    ...
    def _forward_decoder_step(
        ...
        if v >2:
            for i in range(2, v):
                f_state_all, output = self._rnn_decoder_initialize(f_state, feat[:, i], None, None)
