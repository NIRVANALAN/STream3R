...
class FramePack3Rv3(FramePack3R):
    ...
    def _rnn_decoder_initialize_largev(self, f1, pos_1, f2, pos_2, *args, **kwargs):
        # ! initializing causal inference from paired views
        ...
        final_output = [(
            f1 + self.learnable_frame0_token,
            # f2 + self.learnable_B_token,
        ...
        for idx, blk_siamese in enumerate(
            ...
            # memory read all from incoming frames
            f_state, _ = blk_siamese(
                final_state_output[-1],
                torch.cat(
                    [
                        ...
                        rearrange(feat_current[1],
                                  'b v l c -> b (v l) c')  # all frames here
                    ],
                    dim=1),  # b l c
                None,
                None)
        ...
        return final_output, final_state_output[-1]
    ...
    def _rnn_decoder_causal(self,
        ...
        final_output = [
            f_nextpack + self.learnable_B_token.unsqueeze(1)
        ...
        # st()
        mem_feat = [
            rearrange(memory[i][:, memory_indices], 'b v l c -> b (v l) c')
    ...
    def _forward_decoder_step(
        ...
        # if v > 2:
        if False: # make compatible with existing checkpoints
            ress, f_state = self._rnn_decoder_initialize_largev(
                feat[:, 0], None, feat[:, 1:self.pack_size + 1], None)
        else: # init
            (m1, m2), f_state = self._rnn_decoder_initialize(
                feat[:, 0], None, feat[:, 1], None)
            m2 = self._dropout_memory_token(m2)
            # ress.append(m1)
            # ress.append(m2)

            ress = [
                torch.stack([m1[i], m2[i]], dim=1)
                for i in range(self.dec_depth)
            ]
            # st()
            ...
            if self.global_3d_feedback:
                ...
                memory = [
                    ...
                    torch.stack(
                        ...
                        dim=1) for i in range(self.dec_depth) # b v l c
        ...
        if v > 2:
            ...
            for i in range(2, v,
                ...
                # ! update the memory with inj
                for pack_idx in range(0, min(self.pack_size, v-i)):
                    dec = [ress[layer_idx][:, i+pack_idx] for layer_idx in range(self.dec_depth)]
                    ...
                    if self.global_3d_feedback:
                        ...
                        memory = [
                            torch.cat([memory[layer_idx], (dec_for_mem[layer_idx] + feedback_list[layer_idx]).unsqueeze(1)],
                                    dim=1) for layer_idx in range(self.dec_depth)
                        ]  # update the causal memory

                    else:  # directly use dec[i] as the memory. like KV Cache.
                        memory = [
                            torch.stack([memory[layer_idx], dec_for_mem[layer_idx].unsqueeze(1)], dim=1)
                            for layer_idx in range(self.dec_depth)
                        ]  # update the causal memory
