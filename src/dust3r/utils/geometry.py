...
def geotrf(Trf, pts, ncol=None, norm=False):
    ...
    # optimized code
    if (isinstance(Trf, torch.Tensor) and isinstance(pts, torch.Tensor)
            and Trf.ndim == 3 and pts.ndim == 4):
        d = pts.shape[3]
        if Trf.shape[-1] == d:
...
def normalize_pointcloud_group(
    ...
    if norm_mode == "avg":
        ...
        norm_factor = (all_conf * all_dis).sum(dim=1) / (all_conf.sum(dim=1) +
                                                         1e-8)
    else:
...
# https://github.com/btsmart/splatt3r/blob/bda1dd07cf84b9189baabb1b8beac243959cfe5d/utils/geometry.py#L111C1-L141C1
...
def camera_space_to_pixel_space(camera_space_points, intrinsics):
    ...
    pixel_space_points = torch.einsum('b u i j , b v u h w j -> b v u h w i',
                                      intrinsics, camera_space_points)
...
def world_space_to_camera_space(world_space_points, c2w):
    ...
    camera_space_points = torch.einsum('b u i j , b v h w j -> b v u h w i',
                                       c2w.inverse(), world_space_points)
    return camera_space_points[..., :3]


def get_frustum_mask(points_3d, c2w_ctx, intrinsics_ctx, tgt_valid_mask=None):

    # ! create fg mask to stablize the training, as in splatt3r.
    # https://github.com/btsmart/splatt3r/blob/bda1dd07cf84b9189baabb1b8beac243959cfe5d/utils/loss_mask.py#L75

    camera_points = world_space_to_camera_space(
        points_3d, c2w_ctx)  # (b, v1, v2, h, w, 3)
    points_2d = camera_space_to_pixel_space(
        camera_points, intrinsics_ctx)  # (b, v1, v2, h, w, 2)

    # Condition 1: Check if the points are in the frustum of any of the context views
    in_frustum_mask = ((points_2d[..., 0] > 0) & (points_2d[..., 0] < 1) &
                       (points_2d[..., 1] > 0) & (points_2d[..., 1] < 1)).any(
                           dim=-3)  # (b, v_tgt, v_ctx, h, w)

    # Condition 2: apply valid_mask also filtered by depth map
    ress = {'in_frustum_mask': in_frustum_mask}

    if tgt_valid_mask is not None:
        in_frustum_mask_final = tgt_valid_mask & in_frustum_mask
        ress.update({'valid_mask': tgt_valid_mask})

    else:
        in_frustum_mask_final = in_frustum_mask

    ress.update({'in_frustum_mask_final':
                 in_frustum_mask_final.unsqueeze(2)})  # b v 1 h w

    # return in_frustum_mask_final
    return ress
...
def weighted_procrustes(A, B, w, use_weights=True, eps=1e-16, return_T=False):
    ...
    Z[:, -1,
      -1] = torch.sign(torch.linalg.det(U @ V.transpose(1, 2)))  # B x 3 x 3
    R = V @ Z @ U.transpose(1, 2)  # B x 3 x 3
    t = b_mean - torch.einsum("bij,bjk->bik", R, a_mean.transpose(
        -2, -1)).transpose(-2, -1)
    if return_T:
