...
from .utils.image import load_images, rgb
...
def _convert_scene_output_to_glb(outdir, imgs, pts3d, mask, focals, cams2world, cam_size=0.05,
    ...
    per_window_vis = False
    if per_window_vis:
        assert as_pointcloud
        for v, (img_pts, img_col) in enumerate(zip(pts3d, imgs)):
            # pts = np.concatenate([p[m] for p, m in zip(pts3d, mask)])
            # col = np.concatenate([p[m] for p, m in zip(imgs, mask)])
            # pct = trimesh.PointCloud(pts.reshape(-1, 3), colors=col.reshape(-1, 3))
            pct = trimesh.PointCloud(img_pts.reshape(-1, 3), colors=img_col.reshape(-1, 3))

            outfile = os.path.join(outdir, f'ga_pcd_{v}.obj')
            pct.export(outfile, 'obj')
    ...
    if as_pointcloud:
        ...
        outfile = os.path.join(outdir, 'ga_pcd.obj')
        pct.export(outfile, 'obj')
        # st()
...
def get_3D_model_from_scene(outdir, silent, scene, min_conf_thr=3, as_pointcloud=False, mask_sky=False,
    ...
    if min_conf_thr is None: # otherwise, use the default scene.min_conf_thr
