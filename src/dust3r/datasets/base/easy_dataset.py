...
class EasyDataset:
    ...
    def make_sampler(self,
        ...
        if world_size > 1:  # modified according to gpt
            # print('world size: ', world_size, 'global rank: ', rank)

            # 1️⃣ Create DistributedSampler
            distributed_sampler = DistributedSampler(
                self,
                num_replicas=world_size,
                rank=rank,
                shuffle=True,  # This ensures dataset is shuffled across GPUs
                drop_last=drop_last)

            # 2️⃣ Get indices for this GPU
            distributed_indices = list(iter(distributed_sampler))
            sampler.set_indices(
                distributed_indices)  # Assign correct indices to the sampler
