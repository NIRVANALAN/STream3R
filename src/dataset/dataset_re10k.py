...
@dataclass
class DatasetBlendedMVSCfgWrapper:
    blendedmvs: DatasetRE10kCfg  # ! set to the dataset type name to avoid decite error


@dataclass
class DatasetMVICfgWrapper:
    mvi: DatasetRE10kCfg  # ! set to the dataset type name to avoid decite error
...
class DatasetRE10k(IterableDataset):
    ...
    def __init__(
        ...
        # remove bad examples
        dataset_name = cfg.roots[0].stem
        self.dataset_name = dataset_name
        # if 'dl3dv' == dataset_name:

        bad_examples = {}
        # if 'dl3dv' in dataset_name:
        #     with open('datasets_storage/dl3dv_skipped.txt') as f:
        #         bad_examples=[item.strip() for item in f.readlines() ] # remote \n
        # elif 're10k' in dataset_name:
        #     with open(f'datasets_storage/re10k_skipped.txt', 'r') as f:
        #         bad_examples=[item.strip() for item in f.readlines() ] # to read

        self.skipped_instances = set(bad_examples)
    ...
    def __iter__(self):
        ...
        if self.stage == "test" and worker_info is not None:
            self.chunks = [
                chunk for chunk_index, chunk in enumerate(self.chunks)
        ...
        for chunk_idx, chunk_path in enumerate(self.chunks[:]):
            ...
            if self.cfg.overfit_to_scene is not None:
                item = [
                    x for x in chunk if x["key"] == self.cfg.overfit_to_scene
                ]
                assert len(item) == 1
            ...
            for idx, example in enumerate(chunk):  # load a scene
                ...
                if self.cfg.make_baseline_1:  # True
                    a, b = context_extrinsics[0, :3,
                                              3], context_extrinsics[-1, :3, 3]
                    scale = (a - b).norm()
                ...
                crop_shim_example = apply_crop_shim(
    ...
    def data_stage(self) -> Stage:
        ...
        if self.cfg.overfit_to_scene is not None:
            return "test"
        if self.stage == "val":
            return 'vis'  # use the fixed subset for eval
            # return "test"
        return self.stage
...
class DatasetRE10kWindow(DatasetRE10k):
    ...
    def __iter__(self):
        ...
        for chunk_path in self.chunks:
            ...
            for idx, vid_example in enumerate(chunk):  # load a scene
                ...
                extrinsics, intrinsics = self.convert_poses(
                    vid_example["cameras"])
                ...
                for idx, (context_indices, target_indices,
                          overlap) in enumerate(
                              zip(context_indices_list, target_indices_list,
                                  overlap_list)):

                    extrinsics_of_the_window = extrinsics.clone()

                    # Skip the example if the field of view is too wide.
                    if (get_fov(intrinsics).rad2deg()
                            > self.cfg.max_fov).any():
                        continue

                    # Load the images.
                    try:
                        context_images = [
                            vid_example["images"][index.item()]
                            for index in context_indices
                        ]
                        context_images = self.convert_images(context_images)
                        target_images = [
                    ...
                    # st()
                    if self.cfg.skip_bad_shape and (context_image_invalid
                                                    or target_image_invalid):
                        # if self.cfg.skip_bad_shape and (context_image_invalid or target_image_invalid):
                        print(
                            f"Skipped bad example {example['key']}. Context shape was "
                    ...
                    # Resize the world to make the baseline 1.
                    context_extrinsics = extrinsics_of_the_window[
                        context_indices]
                    # st()
                    if self.cfg.make_baseline_1:  # True
                        a, b = context_extrinsics[
                            0, :3, 3], context_extrinsics[-1, :3, 3]
                        scale = (a - b).norm()
                        if scale < self.cfg.baseline_min or scale > self.cfg.baseline_max:
                            print(
                                f"Skipped {scene} because of baseline out of range: "
                    ...
                    if self.cfg.relative_pose:  # True
                        extrinsics_of_the_window = camera_normalization(
                            extrinsics_of_the_window[context_indices][0:1],
                            extrinsics_of_the_window)

                    # st()
                    example = {
                        "context": {
                            "extrinsics":
                            extrinsics_of_the_window[context_indices],
                            "intrinsics": intrinsics[context_indices],
                            "image": context_images,
                            "near":
                            self.get_bound("near", len(context_indices)) /
                            scale,
                            "far":
                            self.get_bound("far", len(context_indices)) /
                            scale,
                            "index": context_indices,
                            "overlap": overlap,
                        },
                        "target": {
                            "extrinsics":
                            extrinsics_of_the_window[target_indices],
                            # "global_extrinsics": global_extrinsics[target_indices],
                            "intrinsics": intrinsics[target_indices],
                            "image": target_images,
                            "near":
                            self.get_bound("near", len(target_indices)) /
                            scale,
                            "far":
                            self.get_bound("far", len(target_indices)) / scale,
                            "index": target_indices,
                    ...
                    if self.stage == "train" and self.cfg.augment:
                        example = apply_augmentation_shim(
                            example)  # reflect views
                    cropped_example = apply_crop_shim(
                        example, tuple(self.cfg.input_image_shape))
                ...
                if return_global_traj:
                    ...
                    scale = vid_window_example_list[0][
                        'scale']  # use the first window input as the global scale.

                    traj_extrinsics = vid_window_example_list[0][
                        'global_extrinsics'].clone(
                        )  # ctx[0] is the canonical frame, not extrinsics[0]
                    intrinsics = vid_window_example_list[0]['target'][
                        'intrinsics'].clone()[0:1].repeat_interleave(
                            len(traj_extrinsics), 0)  # already scaled
                    ...
                    vid_window_example_list.append({
                        ...
                        intrinsics,
                        "near":
                        self.get_bound("near", len(traj_extrinsics)) / scale,
                        "far":
                        self.get_bound("far", len(traj_extrinsics)) / scale,
                        "scene":
                        scene,
                        "window_idx":
                        -1,  # sentinal for global trajectory
                        'scale':
                        scale,
                        "normalize_pts":
                        False,
                        ...
                        'image':
                        torch.cat([
                            vid_window_example_list[i]['target']['image']
                            for i in range(len(vid_window_example_list))
                        ])  # concat the target images
                    })
...
'''
(Pdb) vid_window_example_list[0]['target']['extrinsics'][0]
tensor([[ 0.9984, -0.0067, -0.0563,  0.0918],
        [ 0.0068,  1.0000,  0.0013,  0.0612],
        [ 0.0563, -0.0017,  0.9984, -1.1388],
        [ 0.0000,  0.0000,  0.0000,  1.0000]])
(Pdb) traj_extrinsics[0]
tensor([[ 1.0000e+00,  9.3132e-10,  0.0000e+00,  1.7233e-08],
        [-2.3283e-10,  1.0000e+00,  0.0000e+00, -4.3082e-09],
        [ 1.4901e-08,  9.3132e-10,  1.0000e+00, -6.8931e-08],
