    ...
    with distributed_state.split_between_processes(seq_list) as seqs:
        ...
        for seq in tqdm(seqs):
            ...
            try:
                ...
                # ! 20K tokens here, for videos with 50 frames
                # (Pdb) len(memory)
                # 12
                # (Pdb) memory[0].shape
                # torch.Size([1, 20850, 768])
                # (Pdb) p dec[0].shape
                # torch.Size([1, 417, 768])
                # (Pdb) p dec[1].shape                                                                                                                                                                                             
                # torch.Size([1, 417, 768])
                ...
                print(
                    f"Finished depth estmation of {len(filelist)} images, FPS: {fps:.2f}"
                )
