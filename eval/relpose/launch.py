...
from src.utils import inference_pl

from src.dust3r.utils.geometry import weighted_procrustes, geotrf
...
def recover_cam_params(pts3ds_self, pts3ds_other, conf_self, conf_other):
    ...
    pp = (torch.tensor([W // 2, H // 2],
                       device=pts3ds_self.device).float().repeat(B, 1).reshape(
                           B, 1, 2))
...
def prepare_output(outputs, revisit=1, solve_pose=False):
    ...
    if solve_pose:
        ...
        pr_poses, pr_intrinsics = zip(*[
            pose_encoding_to_extri_intri(pred["camera_pose"][-1].clone(), (H,
                                                                           W))
        ...
        B, H, W, _ = pts3ds_self.shape
        pp = (torch.tensor([W // 2, H // 2],
                           device=pts3ds_self.device).float().repeat(
                               B, 1).reshape(B, 2))
        focal = estimate_focal_knowing_depth(pts3ds_self,
                                             pp,
                                             focal_mode="weiszfeld")
    ...
    with distributed_state.split_between_processes(seq_list) as seqs:
        ...
        for seq in tqdm(seqs):
            try:
                ...
                outputs = inference_pl(views, model, device)
                ...
                save_intrinsics(cam_dict,
                                f"{save_dir}/{seq}/pred_intrinsics.txt")
