    ...
    with torch.no_grad():
        for name_data, dataset in datasets_all.items():
            ...
            log_file = osp.join(save_path,
                                f"logs_0.txt")
                                # f"logs_{accelerator.process_index}.txt")
            ...
            # with accelerator.split_between_processes(list(range(
            if True:
                ...
                for data_idx in tqdm(idxs):
                    ...
                    if model_name == "ours" or model_name == "cut3r":
                        ...
                        with torch.autocast('cuda', enabled=False):
                            start = time.time()
                            # output = model(batch)
                            output = inference_pl(batch, model, device)
                            end = time.time()
                            # preds, batch = output.ress, output.views
                            preds, batch = output['pred'], output['views']
